    ...
    class Program
    {
        ...
        private static void MrWangConnection_OnLogonReply(int Code, string Msg)
        {
            if (Code == 0)
            {
                ...
                MrWangConnection.SubscribeQuote("TXFD9", 0);
            }
            else
            {
                //登入失敗
        ...
        private static void MrWangConnection_OnTradingReply(Reply reply)
        {
            ...
            if (reply.OrderStatus == OrderStatusEnum.Filled)
            {
                if (reply.Side == SideEnum.Buy)
                {
                    if (reply.Qty > 1)
                    {
                        for (int i = 0; i < reply.Qty; i++) { listReplyBuy.Add(new Reply() { Price = reply.Price }); }
                    }
                    else
                    {
                ...
                else
                {
                    if (reply.Qty > 1)
                    {
                        for (int i = 0; i < reply.Qty; i++) { listReplySell.Add(new Reply() { Price = reply.Price }); }
                    }
                    else
                    {
            ...
            if (listReplyBuy.Count > 0 && listReplyBuy.Count == listReplySell.Count)
            {
                ...
                Console.WriteLine($"Total Point:{Totoal} Profit:{Totoal * 200} Tax:{Tax} Amount:{Totoal * 200 - Tax}");
            }
        }
        ...
        private static void MrWangConnection_OnMatchInfo(Match match)
        {
            ...
            BuyThreshold(10895, match, 0);
            SellThreshold(10843, match, 0);

            BuyThreshold(10900, match, 1);
            SellThreshold(10838, match, 1);
        ...
        private static void SellThreshold(double Price, Match match, int pos)
        {
            if (match.MatchPrice <= Price)
            {
                Order order = new Order()
        ...
        private static void ClosePosition(Match match)
        {
            if (Position > 0)
            {
                Order order = new Order()
